...
一个运行在 **DeepSeek Harness (DSH)** 里的动态 Cordis 插件：在侧边栏设置面板中查看 DeepSeek API **账户余额、本月消费、模型 Token 用量与最近 7 天趋势**。
...
**请勿外传、勿截图公开该文件内容。** API Key 与用量 Token 属于敏感凭据，使用者需自行承担本机存储、账号安全与网络请求带来的风险。
...
- Client 半通过 `harness.handle` / `host.call` 与 Host 通信，UI 注册在 `settings.section` 与 `tool.view.cordis` 两个槽位
